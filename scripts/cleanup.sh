...
CRON_MARKER="cron-renew-certs.sh"  # match by script name so spacing doesn't matter

if [ -f "$CRON_FILE" ]; then
    if grep -q "$CRON_MARKER" "$CRON_FILE"; then
        echo "[cleanup] Removing cert renewal cron job(s) from $CRON_FILE..."
        TMP_FILE="$(mktemp)"

        # Remove any line that references the renewal script
        grep -v "$CRON_MARKER" "$CRON_FILE" > "$TMP_FILE"

        # Preserve perms and replace atomically
        chmod --reference="$CRON_FILE" "$TMP_FILE" 2>/dev/null || chmod 600 "$TMP_FILE"
        mv "$TMP_FILE" "$CRON_FILE"
    else
        echo "[cleanup] No cert renewal cron job found in $CRON_FILE (nothing to remove)."
    fi
else
    echo "[cleanup] Cron file $CRON_FILE does not exist (nothing to clean up)."
fi
