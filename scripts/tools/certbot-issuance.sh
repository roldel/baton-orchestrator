...
proj="${1:-}"; [ -n "$proj" ] || { echo "Usage: baton ssl-issue <project> [--staging] [--email you@domain]"; exit 1; }
shift || true
...
for d in $DOMAIN_ALIASES; do domain_args="$domain_args -d $d"; done

staging_flag=""; [ "$STAGING" -eq 1 ] && staging_flag="--staging"
email_args="--register-unsafely-without-email"
[ -n "$EMAIL" ] && email_args="--email $EMAIL --agree-tos"
