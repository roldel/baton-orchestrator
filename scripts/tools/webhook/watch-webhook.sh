...
# --- Process backlog (improved with find + sort) ---
if find "$WATCH_DIR" -name "$PATTERN" -type f -print -quit 2>/dev/null | grep -q .; then
  log "Processing backlog..."
  find "$WATCH_DIR" -name "$PATTERN" -type f -print |
    sort |
...
# --- MAIN LOOP (critical fix: errors to log, not pipe) ---
log "Monitoring for new files..."

inotifywait -m -q --format '%w%f' -e close_write -e moved_to "$WATCH_DIR" 2>>"$LOG" |
